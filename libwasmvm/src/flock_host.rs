//! Flock host: `flock_core::verifier::verify_ligerito` on a 1-thread pool.
//!
//! Rejects the 72-byte `FLCK` digest stub. Accepts CosmWasm envelope v2:
//! `FLCK | prover_id | curve_id | version=2 | 0 | bincode(commitment, proof)`.
//! Canonical identity R1CS (`m=22`, `k_log=6`) matches flock-prover's ligerito
//! roundtrip. Fiat-Shamir domain is `terp-flock/ligerito/v1` || BLAKE3(instances).
...
pub fn host_verify(proof: &[u8], instances: &[u8]) -> ZkResult<()> {
    ...
    let body: LigeritoBody = bincode::deserialize(&proof[HEADER..])
        .map_err(|_| ZkError::new_err("flock: bincode"))?;
...
/// Envelope used by off-chain prover (not Path A digest v1).
#[allow(dead_code)]
pub fn encode_ligerito_proof(commitment: Commitment, proof: R1csProofLigerito) -> Vec<u8> {
    let mut out = vec![0u8; HEADER];
    out[0..4].copy_from_slice(FLCK);
    out[4] = 3;
    out[5] = 7;
    out[6] = VERSION_LIGERITO;
    out.extend(
        bincode::serialize(&LigeritoBody { commitment, proof }).expect("flock body"),
    );
    out
}
